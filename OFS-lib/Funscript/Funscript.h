...
public:
	...
	// Per-script position range for device mapping
	int32_t minPosition = 0;   // Default minimum position
	int32_t maxPosition = 100; // Default maximum position
...
public:
	...
	static std::array<const char*, 10> AxisNames;
	...
	// Position range getters/setters for device mapping
	inline int32_t MinPosition() const noexcept { return minPosition; }
	inline int32_t MaxPosition() const noexcept { return maxPosition; }
	inline void SetPositionRange(int32_t minPos, int32_t maxPos) noexcept { minPosition = minPos; maxPosition = maxPos; }
