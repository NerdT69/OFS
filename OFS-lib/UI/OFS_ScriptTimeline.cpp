...
inline static FunscriptAction getActionForPoint(const OverlayDrawingCtx& ctx, ImVec2 point) noexcept
{
	...
	// Reverse the normalization: convert 0-1 normalized position back to raw 0-100
	float normalizedY = 1.f - relativeY; // 0 at bottom, 1 at top
	float pos;
	if (ctx.maxPos > ctx.minPos) {
		pos = ctx.minPos + normalizedY * (ctx.maxPos - ctx.minPos);
	} else {
		pos = normalizedY * 100.f;
	}
	
	return FunscriptAction(atTime, Util::Clamp<float>(pos, 0.f, 100.f));
}
...
void ScriptTimeline::Init(uint32_t ofsStateHandle)
{
	overlayStateHandle = BaseOverlayState::RegisterStatic();
	stateHandle = ofsStateHandle;
...
bool ScriptTimeline::handleTimelineClicks(const OverlayDrawingCtx& ctx) noexcept
{
	...
	{
		// Click on empty space - clear selection first, then begin new selection
		ctx.DrawingScript()->ClearSelection();
	...
	int activeScriptIdx) noexcept
{
	...
	// Get min/max position range from active script for timeline rendering (fallback to defaults)
	if (activeScriptIdx >= 0 && activeScriptIdx < (int)scripts.size()) {
		drawingCtx.minPos = (float)scripts[activeScriptIdx]->MinPosition();
		drawingCtx.maxPos = (float)scripts[activeScriptIdx]->MaxPosition();
	} else {
		drawingCtx.minPos = 0.f;
		drawingCtx.maxPos = 100.f;
	}
	...
	for(int i=0; i < scripts.size(); i += 1) 
	{
		...
		// Update min/max position range for this specific script
		drawingCtx.minPos = (float)script->MinPosition();
		drawingCtx.maxPos = (float)script->MaxPosition();
		...
		if (ImGui::BeginPopupContextItem(script->Title().c_str()))
		{
			...
			// Edit Min/Max for device position mapping - per-script
			if (ImGui::BeginMenu("Edit Min/Max")) {
				ImGui::Text("Position Mapping for: %s", script->Title().c_str());
				ImGui::Separator();
				
				// Use this script's position range
				static int editMinPos = 0;
				static int editMaxPos = 100;
				static int lastScriptIdx = -1;
				
				// Initialize when opening for this script
				if (lastScriptIdx != i) {
					editMinPos = script->MinPosition();
					editMaxPos = script->MaxPosition();
					lastScriptIdx = i;
				}
				
				ImGui::SetNextItemWidth(ImGui::GetFontSize() * 5.f);
				if (ImGui::InputInt("Min Position", &editMinPos, 1, 10)) {
					editMinPos = Util::Clamp(editMinPos, 0, 100);
				}
				ImGui::SetNextItemWidth(ImGui::GetFontSize() * 5.f);
				if (ImGui::InputInt("Max Position", &editMaxPos, 1, 10)) {
					editMaxPos = Util::Clamp(editMaxPos, 0, 100);
				}
				
				ImGui::Separator();
				
				if (ImGui::Button("Apply", ImVec2(60, 0))) {
					script->SetPositionRange(editMinPos, editMaxPos);
					LOGF_INFO("Position mapping applied to %s: Min=%d, Max=%d", script->Title().c_str(), editMinPos, editMaxPos);
				}
				ImGui::SameLine();
				if (ImGui::Button("Reset", ImVec2(60, 0))) {
					editMinPos = 0;
					editMaxPos = 100;
					script->SetPositionRange(0, 100);
				}
				
				ImGui::Separator();
				ImGui::TextWrapped("Min/Max maps funscript positions to device output range. For squirt (0-1), set Min=0, Max=1.");
