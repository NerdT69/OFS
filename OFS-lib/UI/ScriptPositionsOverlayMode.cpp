...
ImVec2 BaseOverlay::GetPointForAction(const OverlayDrawingCtx& ctx, FunscriptAction action) noexcept
{
    ...
    // Normalize position to 0-1 range based on min/max, then scale to canvas
    float normalizedPos = (ctx.maxPos > ctx.minPos) 
        ? (action.pos - ctx.minPos) / (ctx.maxPos - ctx.minPos) 
        : action.pos / 100.f;
    normalizedPos = Util::Clamp(normalizedPos, 0.f, 1.f);
    
    float y = (ctx.canvasSize.y) * (1.f - normalizedPos);
    x += ctx.canvasPos.x;
    y += ctx.canvasPos.y;
    return ImVec2(x, y);
};
...
void BaseOverlay::drawActionLinesSpline(const OverlayDrawingCtx& ctx, const BaseOverlayState& state) noexcept
{
    auto drawSpline = [](const OverlayDrawingCtx& ctx, FunscriptAction startAction, FunscriptAction endAction, uint32_t color, float width, bool background = true) noexcept
    {
        ...
        auto getPointForTimePos = [](const OverlayDrawingCtx& ctx, float time, float pos) noexcept {
            ...
            float x = (ctx.canvasSize.x) * relative_x;
            // Normalize position to 0-1 range based on min/max
            float normalizedPos = (ctx.maxPos > ctx.minPos) 
                ? (pos - ctx.minPos) / (ctx.maxPos - ctx.minPos) 
                : pos / 100.f;
            normalizedPos = Util::Clamp(normalizedPos, 0.f, 1.f);
            float y = (ctx.canvasSize.y) * (1.f - normalizedPos);
            x += ctx.canvasPos.x;
...
{
    OFS_PROFILE(__FUNCTION__);
    // Draw height indicators based on min/max range
    int lines = 9;
    float range = ctx.maxPos - ctx.minPos;
    if (range <= 1.f) {
        // For 0-1 range (squirt), show just 0 and 1 lines
        lines = 1;
    }
    
    for (int i = 0; i <= lines; i += 1) {
        float normalizedY = (float)i / (float)lines;
        float pos = ctx.minPos + normalizedY * range;
        
        auto color = (pos == 50.f) ? IM_COL32(150, 150, 150, 255) : IM_COL32(80, 80, 80, 255);
        auto thickness = (pos == 50.f) ? 2.f : 1.0f;
        
        float yPos = ctx.canvasPos.y + ctx.canvasSize.y * (1.f - normalizedY);
        ctx.drawList->AddLine(
            ctx.canvasPos + ImVec2(0.0, yPos - ctx.canvasPos.y),
            ctx.canvasPos + ImVec2(ctx.canvasSize.x, yPos - ctx.canvasPos.y),
            color,
            thickness
        );
    }
