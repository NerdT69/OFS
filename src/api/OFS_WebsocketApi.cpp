...
static int http_bluetooth_handler(struct mg_connection *conn, void *cbdata) {
    (void)cbdata;
    
    // Serve the bluetooth-connector.html file
    const char* html_path = "data/bluetooth-connector.html";
    
    // Try to open the file
    FILE* f = fopen(html_path, "rb");
    if (!f) {
        // Try from current directory
        f = fopen("bluetooth-connector.html", "rb");
    }
    ...
    if (f) {
        ...
        if (buffer) {
            ...
            // Send HTTP response
            mg_send_http_ok(conn, "text/html", size);
...
static const char BT_WS_URL[] = "/bluetooth";
...
// Bluetooth WebSocket handlers
static int bt_ws_connect_handler(const struct mg_connection *conn, void *ctx) noexcept
{
    (void)ctx;
    LOG_INFO("Bluetooth WebSocket: New connection");
    
    if (g_BluetoothClient) {
        g_BluetoothClient->InitializeConnection(const_cast<struct mg_connection*>(conn));
    }
    return 0;
}

static void bt_ws_ready_handler(struct mg_connection *conn, void *user_data) noexcept
{
    (void)conn;
    (void)user_data;
    LOG_INFO("Bluetooth WebSocket: Client ready");
    
    if (g_BluetoothClient) {
        g_BluetoothClient->SendMessage("READY");
    }
}

static int bt_ws_data_handler(struct mg_connection *conn, int opcode, char *data, size_t datasize, void *user_data) noexcept
{
    (void)conn;
    (void)user_data;
    
    // Opcode 129 = 0x81 = TEXT (1) with FIN bit (128) set
    // Mask out FIN bit to get actual opcode
    int actualOpcode = opcode & 0x0F;
    if (actualOpcode != 1) {
        return 1;
    }
    ...
    if (g_BluetoothClient) {
        g_BluetoothClient->HandleMessage(msg);
    }
    
    return 1;
}

static void bt_ws_close_handler(const struct mg_connection *conn, void *user_data) noexcept
{
    (void)conn;
    (void)user_data;
    
    LOG_INFO("Bluetooth WebSocket: Connection closed");
    if (g_BluetoothClient) {
        g_BluetoothClient->Close();
    }
}
...
OFS_WebsocketApi::OFS_WebsocketApi() noexcept
{
    ...
    // Initialize Bluetooth WebSocket client
    g_BluetoothClient = new OFS_BluetoothWebSocketClient();
...
bool OFS_WebsocketApi::StartServer() noexcept
{
    ...
    if(!CTX->web) {
        LOGF_ERROR("WebSocket server failed to start on port %s: %s", state.port.c_str(), CTX->errtxtbuf);
        return false;
    }
    ...
    /* Register Bluetooth websocket handler */
    mg_set_websocket_handler_with_subprotocols(CTX->web,
                                               BT_WS_URL,
                                               &wsprot,
                                               bt_ws_connect_handler,
                                               bt_ws_ready_handler,
                                               bt_ws_data_handler,
                                               bt_ws_close_handler,
                                               ctx);
    
    /* Register HTTP handler for bluetooth-connector.html */
    mg_set_request_handler(CTX->web, "/bluetooth-connector", http_bluetooth_handler, ctx);
	return true;
}
