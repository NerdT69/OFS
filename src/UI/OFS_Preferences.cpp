	...
	if (ImGui::BeginPopupModal(TR_ID("PREFERENCES", Tr::PREFERENCES), &ShowWindow, ImGuiWindowFlags_AlwaysAutoResize))
	{
		...
		if (ImGui::BeginChild("prefTabChild", ImVec2(600.f, 360.f))) {
			if (ImGui::BeginTabBar("##PreferenceTabs"))
			{
				...
				if (ImGui::BeginTabItem("Voice Input"))
				{
					ImGui::TextWrapped("Use voice commands to add points to the funscript while playing a video.");
					ImGui::TextWrapped("Say a number (0-100) to add a point at that position.");
					ImGui::Separator();

					if (ImGui::Checkbox("Enable Voice Input", &state.voiceInputEnabled)) {
						save = true;
					}
					OFS::Tooltip("Enable microphone input for voice commands");

					ImGui::BeginDisabled(!state.voiceInputEnabled);
					if (ImGui::InputFloat("Delay (seconds)", &state.voiceInputDelay, 0.1f, 0.5f)) {
						state.voiceInputDelay = Util::Clamp(state.voiceInputDelay, 0.0f, 10.0f);
						save = true;
					}
					OFS::Tooltip("Time offset for when the action is placed (negative = earlier, positive = later)");

					ImGui::Text("Current delay: %.1f seconds", state.voiceInputDelay);
					ImGui::TextWrapped("Say 'fifty' to add a point at position 50, or say 'seventy five' for 75.");
					ImGui::EndDisabled();
